...
const initSocket = (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("call-expired", (data) => {
      ...
      console.log(
        `⏰ Appel expiré sur ${channelName} - Notification à ${receiverId} et ${callerId}`
      );
      ...
      if (receiverId && onlineUsers.has(receiverId)) {
        ...
        console.log(
          `⏰ Notification d'expiration envoyée au destinataire ${receiverId}`
        );
      }
      ...
      if (callerId && onlineUsers.has(callerId)) {
        ...
        console.log(
          `⏰ Notification d'expiration envoyée à l'émetteur ${callerId}`
        );
      }
    });
