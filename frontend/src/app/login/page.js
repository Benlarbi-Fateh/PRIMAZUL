...
export default function LoginPage() {
  ...
  // ✅ Ne rediriger QUE si l'utilisateur est déjà connecté ET qu'on n'est pas en train de se connecter
  useEffect(() => {
    if (user && !loading) {
      router.push("/");
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        // Utilisateur non trouvé (normalement le backend renvoie 401)
